...
from paf.paf_impl import logger, CommunicationMode, ExecutionMode

class AndroidDeploymentTask(BaseAndroidTask):
...
class android_init(AndroidDeploymentTask):
    ...
    def execute(self):
        ...
        if self.has_non_empty_environment_param("ANDROID_LOCAL_MANIFESTS_GIT_URL"):
            ...
            if self.has_non_empty_environment_param("ANDROID_LOCAL_MANIFESTS_GIT_BRANCH"):
                self.subprocess_must_succeed(f"cd {self.ANDROID_SOURCE_PATH}/.repo/local_manifests && git checkout" + " ${ANDROID_LOCAL_MANIFESTS_GIT_BRANCH}")
        else:
            logger.info("ANDROID_LOCAL_MANIFESTS_GIT_URL is not specified. Let's remove existing local manifest")
            self.subprocess_must_succeed(f"cd {self.ANDROID_SOURCE_PATH} && rm -rf .repo/local_manifests")
...
class android_build(AndroidDeploymentTask):
    ...
    def execute(self):
        ...
        self.has_non_empty_environment_param("LOCAL_VENDOR_SOURCE_PATH"):
            if self.has_environment_true_param("LOCAL_VENDOR_COPY_NEWER_FILES"):
                self.subprocess_must_succeed(f"cd {self.ANDROID_SOURCE_PATH} && mkdir -p " + "${LOCAL_VENDOR_DESTINATION_PATH}; cp -ur ${LOCAL_VENDOR_SOURCE_PATH}/* " + f"{self.VENDOR_PATH}")
            else:
        ...
        self.subprocess_must_succeed(f"cd {self.ANDROID_SOURCE_PATH} && . ./build/envsetup.sh; "
                                     ...
                                     "lunch ${ANDROID_LUNCH_CONFIG} && make" + make_target + "-j${BUILD_SYSTEM_CORES_NUMBER}",
...
class android_run_emulator(AndroidDeploymentTask):
    ...
    def execute(self):
        ...
        self.subprocess_must_succeed(f"export OUT_DIR_COMMON_BASE={self.ANDROID_BUILD_DIR}; cd {self.ANDROID_SOURCE_PATH} && . ./build/envsetup.sh && lunch "
                                     "${ANDROID_LUNCH_CONFIG}; " + f"emulator{wipe_data_param} -skin 1280x768 -writable-system -qemu{network}")
...
class android_run_emulator_non_blocking(AndroidDeploymentTask):
    ...
    def execute(self):
        ...
        while attempt < max_attempt:
            ...
            result = self.exec_subprocess("adb shell getprop sys.boot_completed")
