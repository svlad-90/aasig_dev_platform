...
class CTSDeploymentTask(BaseAndroidTask):
    ...
    def execute(self):
        ...
        while attempt < max_attempt:
            ...
            if "emulator" in result:
                ...
                if self.has_non_empty_environment_param("CTS_MODULE"):
                    cts_module = "-m ${CTS_MODULE}"
                # we need to activite Rotary service for cts
                self.subprocess_must_succeed("adb shell settings put secure enabled_accessibility_services com.android.car.rotary/com.android.car.rotary.RotaryService")
                self.subprocess_must_succeed(f"{self.CTS_DEPLOY_PATH}/android-cts/tools/cts-tradefed run cts " + cts_module,
                    communication_mode = CommunicationMode.PIPE_OUTPUT)
